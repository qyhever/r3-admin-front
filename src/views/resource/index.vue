<template>
  <div class="p-[20px]">
    ...
    <OperationDialog ref="operationDialogRef" @create="send" @update="send"></OperationDialog>
  </div>
</template>
...
const columns = ref<TableProps<ResourceItem>['columns']>([
  {
    colKey: 'row-select',
    type: 'multiple',
    width: 50,
...
const onUpdate = (row: ResourceItem) => {
  operationDialogRef.value?.open(row)
}
...
const onDelete = async (id: number) => {
  try {
    ...
    const newData = dataSource.value.filter((item: ResourceItem) => item.id !== id)
    update({
      data: newData,
    })
  } catch (error) {
...
const onBatchDelete = async () => {
  ...
  await batchDeleteResources(selectedRowKeys.value)
  MessagePlugin.closeAll()
